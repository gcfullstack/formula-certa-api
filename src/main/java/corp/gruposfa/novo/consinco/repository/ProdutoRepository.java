...
public interface ProdutoRepository extends JpaRepository<NotaFiscal,Integer>{
	...
	@Query(value = "SELECT * FROM BI_V_DADOS_QUERODELIVERY where cod_loja = :idLoja", nativeQuery = true)
	List<ProdutoInterfaceDTO> buscarProdutos(@Param("idLoja") Integer idLoja);
