...
public interface CategoriaRepository extends JpaRepository<NotaFiscal,Integer>{
	...
	@Query(value = "SELECT DISTINCT NOME_CATEGORIA as nomeCategoria,COD_CATEGORIA as codCategoria FROM BI_V_DADOS_QUERODELIVERY where cod_loja = 2", nativeQuery = true)
	List<CategoriaCompareInterfaceDTO> buscarNomeCategorias();
